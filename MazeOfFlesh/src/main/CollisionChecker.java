...
public class CollisionChecker {
	...
	public void checkTile(Entity entity) {
		...
		// USE TEMP DIRECTION WHEN IT'S BIENG KNOCKBACKED
	...
	public int checkObject(Entity entity, boolean player) {
		...
		for (int i = 0; i < gp.obj[1].length; i++) {
			...
			if (gp.obj[gp.currentMap][i] != null) {
				...
				entity.direction = entity.normalizeDirection(entity.direction);
	...
	public int checkEntity(Entity entity, Entity[][] target) {
		...
		// USE TEMP DIRECTION WHEN IT'S BIENG KNOCKBACKED
		String direction = entity.normalizeDirection(entity.direction);
		if (entity.knockBack == true) {
			direction = entity.knockBackDirection;
		}
	...
	public void checkMapBoundary(Entity entity) {
	    ...
	    // Calculate the entity's boundaries including its solid area
	    int entityLeftWorldX = entity.worldX + entity.solidArea.x;
	    int entityRightWorldX = entity.worldX + entity.solidArea.x + entity.solidArea.width;
	    int entityTopWorldY = entity.worldY + entity.solidArea.y;
	    int entityBottomWorldY = entity.worldY + entity.solidArea.y + entity.solidArea.height;
	    
	    // USE TEMP DIRECTION WHEN IT'S BEING KNOCKBACKED
	    String direction = entity.direction;
	    if (entity.knockBack) {
	        direction = entity.knockBackDirection;
	    }
	    ...
	    switch (direction) {
	        case "up":
	            int nextTopY = entityTopWorldY - entity.speed;
	            if (nextTopY < 0) {
	                entity.collisionOn = true;
	            }
	            break;
	        case "down":
	            int nextBottomY = entityBottomWorldY + entity.speed;
	            if (nextBottomY > maxWorldRow * tileSize) {
	                entity.collisionOn = true;
	            }
	            break;
	        case "left":
	            int nextLeftX = entityLeftWorldX - entity.speed;
	            if (nextLeftX < 0) {
	                entity.collisionOn = true;
	            }
	            break;
	        case "right":
	            int nextRightX = entityRightWorldX + entity.speed;
	            if (nextRightX > maxWorldCol * tileSize) {
	                entity.collisionOn = true;
	            }
	            break;
	    }
