...
public class CutsceneManager {
	...
	public void scene_something() {
		...
		if(scenePhase == 0) {
			...
			for(int i = 0; i < gp.npc[1].length; i++) {
				// check if vacant, then add the dummy, this is if you want to move the camera in the screen
			...
			// This is for if you want to move the camera and you will make the player disappear
	...
	public void scene_ending() {
		...
		if(scenePhase == 5) {
			...
			// --- add xD
		    try {
		    	bg = ImageIO.read(getClass().getResourceAsStream("/maps/gorila.png"));
		    } catch (Exception e) {
	        e.printStackTrace();
		    }
			...
			String text = "Lets make this short shall wee? \n"
					+ "I don't know why you step on a shit but.\n"
					+ "Guess what?\n"
					+ "You winn!!!\n"
					+ "<(^-^)>";
